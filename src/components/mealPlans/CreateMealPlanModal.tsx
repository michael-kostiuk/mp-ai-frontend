...
const CreateMealPlanModal: React.FC<CreateMealPlanModalProps> = ({
  ...
  const handleAutoGenerateNumericChange = (field: string, stringValue: string) => {
    if (stringValue === '') {
      setAutoGenerateData(prev => ({ ...prev, [field]: '' as any }));
      return;
    }
    const numValue = Number(stringValue);
    if (!isNaN(numValue)) {
  ...
  const updateMealEntryNumeric = (index: number, field: keyof MealPlanEntryCreate, stringValue: string) => {
    if (stringValue === '') {
      updateMealEntry(index, field, '' as any);
      return;
    }
    const numValue = Number(stringValue);
    if (!isNaN(numValue)) {
      updateMealEntry(index, field, numValue);
    }
  };
